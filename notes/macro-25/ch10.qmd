...
#| echo: false
#| output: asis
Markdown(f'![]({puml.get_url(uml)}){{width=100% fig-align="center"}}')
```
...
$$
\begin{alignat}{1}
\mathcal{L} =& u(c_1) + \beta u(c_2) + \lambda_1[(1 + r_1)k_{c0} + w_1L + b_{c1} - c_1 - k_{c1}] \\
              &+ \lambda_2[(1 + r_2)k_{c1} + w_2L - (1 + r_{g2})b_{c1} - c_2]
\end{alignat}
$$
...
::: {.callout-tip}
由$r_2 = r_{g2}$可知，无论居民是通过借出资金、或是接入资金作为投资，[只有在两种方式的收益率想等时，居民消费储蓄才能达到最佳效用]{.text-primary}，否则，居民可以通过向收益率高的风向转移资源配置以获取更高的收益率，而随着收益更高的方向资源变多，其边际收益率会不断降低，直至两者收益率相等
:::
